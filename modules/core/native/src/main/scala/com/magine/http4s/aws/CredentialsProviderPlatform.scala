...
private[aws] trait CredentialsProviderPlatform {
  ...
  def default[F[_]: Async](client: Client[F]): Resource[F, CredentialsProvider[F]] =
    for {
      credentialsFile <- credentialsFile.toResource
      containerEndpoint <- containerEndpoint(client)
  ...
  def securityTokenService[F[_]: Async: LiftIO](
    client: Client[F]
